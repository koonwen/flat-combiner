...
module Stack_ext = struct
  ...
  let global_steal ~from ~to_local = steal ~from ~to_local ()
end
...
module Hybrid_random = Scheduler.Make (struct
  ...
  let local_remove t =
    if Random.int 20 > 9 then steal ~from:t ~steal_size_limit:1 ~to_local:t () |> ignore;
    local_pop t
  ;;
...
module Hybrid_alternating = Scheduler.Make (struct
  ...
  let local_remove t =
    ...
    then steal ~auto_retry:true ~steal_size_limit:1 ~from:t ~to_local:t () |> ignore;
...
module FIFO_with_slot = Scheduler.Make (struct
  ...
  let local_insert queue item =
    ...
    | None ->
      ...
      true
    | Some old_item ->
      if local_enqueue queue old_item
      then (
        set_slot queue (Some item);
        true)
      else false
