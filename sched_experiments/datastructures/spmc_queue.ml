...
let set_slot { slot; _ } new_item = slot := new_item
...
(* Cautionary check because debugging broken 'local' invariant 
  is hard. *)
let register_domain_id { owned_by_id; _ } = owned_by_id := Some (Domain.self ())
...
let local_resize t =
  ...
  let new_array_val = Array.init (size * 2) (fun _ -> Atomic.make None) in
...
let rec local_enqueue_with_resize t element =
  ...
  | Some _ ->
    ...
    if Option.is_some (Atomic.get cell) then local_resize t;
...
let local_is_empty queue =
  ...
  let tail_val = Atomic.get tail in
  let head_val = Atomic.get head in
  if tail_val > head_val then false else local_is_empty_thorough queue
;;

let get_id { owned_by_id; _ } =
  match !owned_by_id with
  | None -> -1
  | Some v -> (v :> int)
;;
...
let steal ~from ~to_local =
  (* Printf.printf "Stealing from (%d) to (%d)\n%!" (get_id from) (get_id to_local); *)
  ...
  then 0
    ...
    then 0
      ...
      for i = 0 to stealable - 1 do
        ...
        Atomic.set cell None;
        while not (local_enqueue to_local (value_exn !value)) do
          ()
        done
      done;
      stealable))
...
let indicative_size { head; tail; _ } = max (Atomic.get tail - Atomic.get head) 0
