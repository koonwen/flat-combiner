...
type _ eff += Yield : bool -> unit eff
type _ eff += Await : 'a Promise.t -> 'a eff
type _ eff += Schedule : (unit -> 'a) -> 'a Promise.t eff

let yield ?(lock_holding = false) () = perform (Yield lock_holding)
...
module Make (DS : DataStructure) = struct
  ...
  module Processor = struct
    ...
    let init ?size_exponent id =
      ...
      ; locked_tasks = Queue.create ()
      }
    ;;

    let id { id; _ } = id
    let ds { ds; _ } = ds
    let incr_tasks { executed_tasks; _ } = executed_tasks := !executed_tasks + 1
    let executed_tasks { executed_tasks; _ } = !executed_tasks
    let zero_executed_tasks { executed_tasks; _ } = executed_tasks := 0
    let zero_steal_attempts { steal_attempts; _ } = steal_attempts := 0
    let steal_attempts { steal_attempts; _ } = !steal_attempts
    ...
    let take_from { steal_attempts; suggest_steal; _ } global_requesting_queue =
      ...
      let steal_or_f v =
        ...
        if !steal_attempts mod 10 == 0 then v else `Steal `Random
      in
      ...
      | (Steal_and_simple_request | Steal_and_sticky_request) as v ->
        let kind =
          ...
          | None -> `Random
          | Some id ->
            if v == Steal_and_simple_request then Atomic.set suggest_steal None;
            `Force id
        in
        `Steal kind
      | Steal_and_overflow_queue -> steal_or_f `Global_queue
      | Steal_and_mpmc_overflow -> steal_or_f `Global_mpmc_queue
      | Steal_and_multi_mpmc_overflow -> steal_or_f `Global_multi_queue
      | Steal_and_advanced_request ->
        let kind =
    ...
    let set_suggest_steal { suggest_steal; _ } id = Atomic.set suggest_steal (Some id)
  end
  ...
  let domain_key = Domain.DLS.new_key (fun () -> None)
  ...
  let with_processor f = with_context (fun ({ processor; _ } : Context.t) -> f processor)
  ...
  let localized_random_id ({ processor; all_processors; _ } : Context.t) min_reachable =
    ...
    then None
      ...
      if r == my_id then None else Some r)
  ;;
  ...
  let schedule_internal ~has_yielded task =
    with_context (fun context ->
      ...
      let insert_f =
        ...
        if has_yielded then DS.local_insert_after_preemption ds else DS.local_insert ds
      in
      ...
      then deal_with_lack_of_space_to_enqueue context task ~insert_f)
  ;;
  ...
  let schedule_awaiting to_run result =
    ...
    | _ ->
      List.iter
        (fun awaiting ->
          schedule_internal ~has_yielded:false (Task.new_task (fun () -> awaiting result)))
        to_run
  ;;
      ...
      { effc =
          (fun (type a) (e : a eff) ->
              ...
              Some
                (fun (k : (a, unit) continuation) ->
                  ...
                  schedule_internal
                    ~has_yielded:false
                    (Task.new_task (fun () ->
                       with_processor (fun processor -> Processor.incr_tasks processor);
                       let result = new_f () in
            ...
            | Yield lock_holding ->
              Some
                (fun k ->
                  with_context (fun { global_queue; processor; _ } ->
                    let task = Task.new_task (fun () -> continue k ()) in
                    if lock_holding
                    then Queue.add task processor.locked_tasks
                    else Custom_queue.enqueue global_queue task))
            | Await promise ->
              Some
                (fun k ->
  ...
  let steal ~context ~id_type =
    ...
    | Some other_queue_id ->
      ...
      DS.global_steal ~from:other_ds ~to_local:my_ds |> ignore
  ;;
  ...
  let take_from_global_queue ~context =
    ...
    | None -> ()
    | Some task -> assert (DS.local_insert ds task)
  ;;
  ...
  let init
    ?(afterwards = `join_the_pool)
    ?overflow_size_exponent
    ?size_exponent
    ...
    in
    let all_processors =
      List.init num_of_processors (fun id -> Processor.init ?size_exponent id)
      |> Array.of_list
    in
    ...
    match afterwards with
    | `return -> { global_queue; all_processors }
    | `join_the_pool ->
  ...
  let pending_tasks { all_processors; _ } =
    Array.fold_right
      (fun processor curr -> DS.indicative_size (Processor.ds processor) + curr)
      all_processors
      0
  ;;
  ...
  module Stats = struct
    let unsafe_print_executed_tasks () =
      with_context (fun { all_processors; _ } ->
        let counters =
          Array.map (fun processor -> Processor.executed_tasks processor) all_processors
        in
    ...
    let unsafe_print_waited_for_space_on_enque () =
      with_context (fun { all_processors; _ } ->
        let counters =
          Array.map
            (fun processor -> Processor.waited_for_space_on_enque processor)
            all_processors
        in
    ...
    let unsafe_print_steal_attempts () =
      with_context (fun { all_processors; _ } ->
        let counters =
          Array.map (fun processor -> Processor.steal_attempts processor) all_processors
        in
        counters
        |> Array.to_list
        |> List.fold_left ( + ) 0
...
module type S = sig
  ...
  val init
    :  ?afterwards:[ `join_the_pool | `return ]
