module FC_Queue = struct
  ...
  let _q = Queue.create ()
  let _fcq = Fc_generic_effects.create ~data_structure:_q ~num_domains

  let enq v =
    match
      Fc_generic_effects.apply _fcq (fun () ->
        Queue.push v _q;
        None)
    with
    | None -> ()
  ...
  let deq () = Fc_generic_effects.apply _fcq (fun () -> Queue.take_opt _q)
